...
class RLTBenchmarkPipeline:
    ...
    def _get_models(self):
        if self.problem_type == "classification":
            return {
                ...
                "ExtraTrees": ExtraTreesClassifier(n_estimators=100, random_state=self.random_state, n_jobs=-1),
                "GradientBoosting": GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, random_state=self.random_state),
                "XGBoost": XGBClassifier(
                    n_estimators=100, max_depth=6, learning_rate=0.1,
                    random_state=self.random_state, n_jobs=-1, verbosity=0
                ),
                "Lasso/Linear": LogisticRegression(
                    max_iter=1000,
                    random_state=self.random_state
                ),
            }
            ...
            return {
                ...
                "ExtraTrees": ExtraTreesRegressor(n_estimators=100, random_state=self.random_state, n_jobs=-1),
                "Lasso/Linear": Lasso(alpha=0.1, max_iter=1000, random_state=self.random_state),
    ...
    def _compute_rlt_variable_importance(self, X_train, y_train,
                                         vi_extra_weight=0.5, vi_stat_weight=0.5,
    ...
    def _progressive_muting_evaluation(
        ...
        for i in range(max_i + 1):
            features_to_mute = features_all[:i]
            features_kept = [f for f in X_train_df.columns if f not in features_to_mute]
            
            if i > 0:
                print(f"   [Muting Step {i}] Muted: {features_all[i-1]} | Remaining: {len(features_kept)}")
    ...
    def benchmark_optimized(self, df, target_col=None, top_n=15, plot=False,
        ...
        def create_linear_combinations_hybrid(X, vi_scores, threshold=50, top_cap=10):
            ...
            if p_m > threshold:
                top_n_feat = min(top_cap, max(2, int(np.sqrt(p_m))))
                rule = "square-root rule (high dimensionality)"
            else:
                top_n_feat = min(p_m, top_cap)
            ...
            print(f"Number of features selected for combinations (top_n): {top_n_feat}")

            # STEP 2: Rank features
            vi_scores = vi_scores.sort_values("VI_Aggregate", ascending=False).reset_index(drop=True)
            top_features = vi_scores.head(top_n_feat)["Feature"].tolist()

            if len(top_features) < 2:
                print("⚠️ Not enough features to create combinations")
                return X_combined

            # STEP 3: Decide how many to keep
            total_combinations = top_n_feat * (top_n_feat - 1) // 2
            ...
            for i in range(len(top_features)):
                for j in range(i + 1, len(top_features)):
                    ...
                    if f1 not in X.columns or f2 not in X.columns:
                        continue
                    w1 = vi_scores.loc[vi_scores["Feature"] == f1, "VI_Aggregate"].values[0]
                    w2 = vi_scores.loc[vi_scores["Feature"] == f2, "VI_Aggregate"].values[0]
                    total_weight = w1 + w2
                    ...
                    X_combined[new_feature_name] = w1_norm * X[f1] + w2_norm * X[f2]
                    print(f"  {new_feature_name} = {w1_norm:.3f} * {f1} + {w2_norm:.3f} * {f2}")
        ...
        for name, model in embedded_models.items():
            ...
            X_train_rlt_opt = create_linear_combinations_hybrid(X_train_mut, vi_df)
            X_test_rlt_opt = create_linear_combinations_hybrid(X_test_mut, vi_df)
